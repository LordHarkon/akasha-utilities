...
export const avatarURL = (email: string, options: GravatarOptions) => {
  ...
  if (options?.s || options?.size) {
    if (options?.s < 1 || options?.size)
      throw new Error("Size cannot be smaller than 1px");
    if (options?.s > 2048 || options?.size > 2048)
      throw new Error("Size cannot be bigger than 2048px");
  }

  if (options?.d)
    if (!DefaultOptions.includes(options.d))
      throw new Error(`${options.d} is not a valid default option`);
